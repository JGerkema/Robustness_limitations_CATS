...
study_lwc <- c("Aiello1992", "Aiello2011", "Buzzard", "Castro", "Chalmandrier", "Choler", "Eallonardo", "Frenette2009", "Frenette2010", "Gonzalez", "Kober", "Jager", "Jäschke", "Asefa", "Kembel", "Liane", "Makelele", "Marteinsdottir", "Osuri", "Raevel", "Riva", "Rolhauser", "Standen"
)
...
check <- classification %>%
  ...
  select(scientificName, acceptedNameUsageID, nomenclaturalStatus) %>%
...
species_list_complete <- data.frame()
...
for(i in 1:length(data_input_list)){
  ...
  extract <- included_now %>%
                filter(study == study_lwc[[i]])
  
  species <- species %>%
    filter(Name %in% extract$Name)
  
  
  species_list_complete <- bind_rows(species_list_complete, species)
...
check2 <- included_now %>%
  filter(Name %in% species_list_complete$Name)

check3 <- trait_imp_specieslvl %>%
  filter(genus == "Carex")

``` 


```{r trait names change}

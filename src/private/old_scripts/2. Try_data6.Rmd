...
test <- WorldFlora::data(vascular.families)
...
try_clean2 <- try_clean %>%
  ...
  # things like: "herbaceaus", "fern", or "ANANI".
...
temp <- matched_clean %>%
  relocate(Old.status, Old.name)

# For 2380 species names, a valid match is found. 
matched_clean_old <- matched_with_wf %>%
  filter(Matched == TRUE) %>%
...
matched_clean <- matched_with_wf %>%
  ...
  filter(nomenclaturalStatus == "Valid") %>% # Check, is dit consistent met de code erna? 
  # Omdat ik hiermee de unchecked namen er ook uit gooi?
...
matched_clean2 <- matched_clean %>%
  select(c(taxonID, scientificName, spec.name.ORIG, genus, family)) %>%
  rename(new_name = scientificName, AccSpeciesName = spec.name.ORIG,
         acceptedNameUsageID = taxonID) 


matched_clean2_new <- matched_clean %>%
  select(c(taxonID, scientificName, spec.name.ORIG, genus, family)) %>%
  rename(AccSpeciesName = spec.name.ORIG) %>%
  left_join(classification)
  #%>%
   #mutate(acceptedNameUsageID = recode(acceptedNameUsageID, "wfo-0000667939" = "wfo-0001067485"))

# I will create a new try_clean dataset, where the names that originally did not 
# have a match with the wfo classification database are swapped with their matched names.
# First, I will extract the entrees that did have a direct match. 

try_direct_match <- try_clean2 %>%
  filter(AccSpeciesName %in% c(try_wf_clean2$scientificName)) 

# Second, I will extract the names that now have new name, obtained by fuzzy matching.
try_fuzzy_matched <- try_clean2 %>%
  filter(AccSpeciesName %in% c(matched_clean2$AccSpeciesName)) %>%
  full_join(matched_clean2) %>%
  select(-c(genus, family, AccSpeciesName)) %>%
  rename(AccSpeciesName = new_name)

temp <- bind_rows(try_direct_match, try_fuzzy_matched)

table(temp2$taxonomicStatus)

length(unique(try_fuzzy_matched$AccSpeciesName))

length(unique(matched_clean2$AccSpeciesName))

temp2 <- matched_with_wf %>%
  relocate(Fuzzy.dist, spec.name.ORIG, scientificName, Subseq) %>%
  filter(taxonRank == "species") %>%
  group_by(spec.name.ORIG) %>%
  filter(n() > 1)
...
check <- not_included_in_wf %>%
  filter(AccSpeciesName %in% c(matched_clean2$AccSpeciesName))
table(temp2$Subseq)
check <- matched_clean2 %>%
  filter(AccSpeciesName %notin% c(try_clean_new_matched$AccSpeciesName)) %>%
  gr


accepted_species2 <- try_wf_clean2 %>%
  filter(taxonomicStatus != "Synonym") %>%
  select(scientificName, taxonID, genus, family, taxonomicStatus) %>%
  distinct() %>%
  group_by(scientificName) %>%
  filter(n() > 1)
table(accepted_species2$taxonomicStatus)
...
# actually a synonym for another species. 
...
# to multiple WFO names, are saved. 
...
synonyms <- try_wf_clean2 %>% 
  ...
  filter(scientificName %notin% c(accepted_species$AccSpeciesName)) %>%  # n = 4160
...
check <- matched_synonyms %>%
  filter(new_name %in% c(accepted_species_multiple$new_name)) %>%
  select(AccSpeciesName, SpeciesName) %>%
  distinct()
...
matched_clean3 <- matched_clean2 %>%
  ...
  rename(acceptedNameUsageID = acceptedNameUsageID.x,
         ...
         genus = genus.x,
         family = family.x)
# TO DO FOR 14-12. check what happens here exactly. And fix it.
...
combined <- bind_rows(try_direct_match2, matched_clean_final)
...
hierarchy <- lookup_table(final_combined$new_name, genus_column = "genus", by_species=F, 
...
# 
 try_tax <- full_join(final_combined, hierarchy) %>%

...
######## Calculate community weighted means and community weighted variance for multiple traits
prep_studylvl <- function(input_veg, input_trait, trait_information, cutoff_value){
  ...
  output <- lst(relative_abundances, trait_data, calculations_cwm_cwv)
  return(output)
}


prep_plotlvl <- function(input_veg, input_trait, trait_information, cutoff_value){
    #' @title Necessary preparations and calculations to create input-ready data for the CATS model - traits at the plot level. 
    #' @description First, the raw abundance data is converted to relative abundances. 
    #' Only the plots with enough trait coverage are retained in the dataset using the 'calculate_abundance' function. 
    #' Then, the community weighted mean (cwm) and variance (cwv) of each plot for multiple traits is calculated.
    #' @param input_veg The vegetation input should be a matrix where each row represents a plot and each column a species. The species columns should be alphabetically ordered. 
    #' @param input_trait The trait input should be a dataframe where each row represents a species. The species names should be stored in a column called "Name". The corresponding trait values should be stored in the rest of the columns, in alphabetical order. 
    #' @param trait_information List of with the trait information. Should contain a list of the trait names, future cwm_names and cwv_names.
    #' @param cutoff_value In some occurrences, not all species will have trait data. These species are removed. The total relative abundance of species with trait data is then calculated. If it exceeds the specified cutoff value, the plot remains in the data. Otherwise it is removed. 
    #' @returns A list with the relative abundances and the calculated CWM and CWV values for each plot. 
    #'  
...
######## Run the CATS model for one plot -- trait data at study level available

cats_studylvl <- function(prep_output, trait_information){
  #' @title CATS model for datasets with species traits at the study level
  #' @description The CATS model is ran for one plot.
  #' @param input_plot The plot for which the CATS model is ran. 
  #' @returns A dataframe with the CATS output.
...
######## Run the CATS model for one plot -- trait data at plot level available

cats_plotlvl <- function(prep_output, trait_information){
...
# Source: https://github.com/djvanderlaan/rijksdriehoek/blob/master/R/rijksdriehoek.R
  ...
  if(missing(x_limits) || missing(y_limits)) {
    ...
    if(length(x_limits) != 2 || length(y_limits) != 2 ||
      ...
      map_country_theme <- theme_bw() +
        theme(panel.background = element_rect(fill = '#cee1f8'),

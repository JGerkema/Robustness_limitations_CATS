...
# Hier wordt de gecleande try database gematcht met de taxonomic dataset,
...
# unieke namen waren geweest dan had ik ze gewoon samenkunnen voegen. Maar 
# dat is niet het geval. Dus ik moet opzoek naar de laatste duplicaten. 
not_included <- try_tax_clean %>%
  filter(AccSpeciesName %notin% c(filter$AccSpeciesName)) 

length(unique(not_included$AccSpeciesName))

# Hiermee heb ik een dataset met daarin de overgebleven duplicates
test_duplicates <- try_tax_clean %>%
  filter(AccSpeciesName %in% c(not_included$AccSpeciesName)) %>%
  count(AccSpeciesName) %>%
  filter(n > 1)

selected_duplicates <- try_tax_clean %>%
  filter(AccSpeciesName %in% c(test_duplicates$AccSpeciesName)) 
2-1

# Hiermee haal ik de unieke soorten eruit, die ga ik fuzzy matchen met de 
# WF dataset 
species_match <- test_duplicates %>% 
  distinct() %>%
  pull(AccSpeciesName)

wfo_match <- WFO.match(spec.data = species_match, WFO.data = classification)
...
##########
try_taxonomic_test <- try_taxonomic %>%
  filter(Genus %in% c("Unknown", "unknown")) 

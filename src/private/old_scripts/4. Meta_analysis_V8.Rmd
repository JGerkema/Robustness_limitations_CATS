...
data_input_list2 <-  lst(kembel_kinsella_output, kembel_onefour_output,
                        frenette_missour_output,
                        frenette_enjil_output, frenette_lamjalil_output,
                        frenette_maatarka_output, frenette_tirnest_output,
...
```{r missing na per study}

traits_na <- traits_rmse <- data.frame()
...
for(i in 1:length(data_input_list)){
 ...
 extract <- new_names_synonyms %>%
  ...
  input_veg <- input_veg[, which(colSums(input_veg) > 0)]
  
    try_traits <- traits_not_imp_specieslvl %>%
    filter(TraitName %in% traitlist) %>%
    filter(Name %in% colnames(input_veg)) %>%
...
if (nrow(missing_traits) > 0) {
  ...
  traits_try_orig <- input_trait %>%
    ...
    filter(Plot_code %in% rownames(input_veg)) %>%
    mutate(across(where(is.numeric), ~ c(scale(., center = F)))) %>%
...
#Als TRY het slecht doet. Komt dat door TRY of door de imputaties?    
...
traits_na2 <- traits_na %>%
  ...
  mutate(study = case_when(study %in% study_names_df$study ~ 
                       ...
                       study_names_df$study)], TRUE ~ study)) %>%
  filter(study %notin% c("Gonzalez (Ca)", "Gonzalez (Ch)", "Gonzalez (Pa)", "Osuri (Fragment)")) %>%
  group_by(study) %>%
  summarize(across(where(is.numeric), ~mean(., na.rm = T))) %>%
  mutate(perc_na = perc_na*100)

#write_xlsx(traits_na2, path = "traits_na.xlsx")


traits_rmse2 <- traits_rmse %>%
 mutate(study_group = case_when(study %in% study_names_df$study ~ 
                       study_names_df$study_group[match(study,
                       study_names_df$study)], TRUE ~ study))  %>%
...
temp_plot <- traits_rmse2 %>%
  full_join(traits_na2) %>%
  relocate(study, RMSE, perc_na)
...
traits_rmse4 <- traits_rmse3 %>%
  mutate(study_group = case_when(study == "Liane_finland_1"  ~ "Liane",
                                 ...
                                 study == "Kembel_kinsella"  ~ "Kembel",
                                 study == "Kembel_onefour"  ~ "Kembel",
                                 study == "Frenette2010" ~ "Frenette",
                                 study == "kober" ~ "Kober",
                                 study == "Frenette_missour" ~ "Frenette",
                                 study == "Frenette_enjil" ~ "Frenette",
                                 study == "Frenette_lamjalil" ~ "Frenette",
                                 study == "Frenette_maatarka" ~ "Frenette",
                                 study == "Frenette_tirnest" ~ "Frenette",
                                 study == "Gonzalez_ma" ~ "Gonzalez",
                                 study == "Gonzalez_ch" ~ "Gonzalez",
                                 study == "Gonzalez_ca" ~ "Gonzalez",
                                 study == "Gonzalez_za" ~ "Gonzalez",
                                 study == "Gonzalez_pe" ~ "Gonzalez",
                                 study == "Gonzalez_en" ~ "Gonzalez",
                                 study == "Gonzalez_pa" ~ "Gonzalez",
                                 study == "Osuri_control" ~ "Osuri",
                                 study == "Osuri_fragment" ~ "Osuri",
                                 TRUE ~ study)) %>%
 filter(study %notin% c("Gonzalez_ca", "Gonzalez_ch", "Gonzalez_pa", 
                         "Gonzalez_ca", "Gonzalez_pe",  "Osuri_fragment")) %>%
  mutate(SQ_ER = (Orig_values - Try_values)^2) %>%
    ...
    group_by(plot_study, study_group, imp) %>%
    summarise(MSE = mean(SQ_ER)) %>%
    ungroup() %>%
...
ggplot(traits_rmse4, aes(x = imp, y = (RMSE))) +
  geom_boxplot(aes(fill = imp)) +
   labs(title = "RMSE for imputated data vs. original TRY data",
       x = NULL,
       y = "RMSE",
       fill = "Imputated") +
  theme_minimal() +
...
ggplot(temp_plot, aes(x = RMSE, y = perc_na, colour = study)) + 
         geom_point(size = 3) +
         theme_minimal()+ 
         labs(x = "RMSE", y = "Missing data (%)", colour = "Dataset",
...
for(i in 1:length(data_input_list2)){
  
  dataset <- data_input_list2[[i]]
  ...
  regional_trait_coverage <- input_processed_orig[["relative_abundances"]][["matrix"]] %>% 
    ...
    plot_count <- bind_cols(`Plots before (n)` = n_plot_before, 

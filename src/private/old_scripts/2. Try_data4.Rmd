...
matched_clean2 <- matched_clean %>%
  select(c(taxonID, scientificName, spec.name)) %>%
...
### AAAAAAAAAAAAAAh het lukt vandaag niet. Mijn hoofd is wazig, warrig, snel 
# afgeleid. Ik weet het even niet. Ik heb te veel energie in mijn lijf, te veel
# onrust. Een gejaagd gevoel. Heb ik teveel suiker gegeten, te weinig water gedronken
# ik weet het niet. ik wil dit zo graag afmaken, ik wil zo graag een stap verder zijn.
# maar mijn hoofd loopt even vast. Alsof hij te vol zit. Ik wil afleiding, maar de tijd
# kruipt en vliegt. Ik wil in de zone zitten maar het lukt niet eens om te beginnen. 

# Er zitten dubbele entrees in. Maar maakt dat uit. Ik weet het niet. 


accepted_species <- try_wf_clean2 %>%
  filter(taxonomicStatus != "Synonym") %>%
  relocate(taxonRank, acceptedNameUsageID, taxonomicStatus) %>%
...
synonyms <- try_wf_clean2 %>% # & !any(nomenclaturalStatus == "Illegitimate")
  filter(taxonomicStatus == "Synonym") %>%
...
single_synonyms <- synonyms %>%
  select(c(scientificName, acceptedNameUsageID)) %>%
  distinct() %>%
...
combined <- bind_rows(matched_synonyms_filtered, accepted_species)

length(unique(combined$AccSpeciesName))
...
# and not a synonym. 
# In most cases, the old and new name are the same. But there are also some 
...
final_pls_be_final <- full_join(eindelijk_klaar, final_taxonomic_information) %>%
  drop_na()

final_species_list <- final_pls_be_final %>%
  select(new_name, genus) %>%
  distinct() #61334

hierarchy <- lookup_table(final_species_list$new_name, genus_column = "genus", by_species=F, 
                          family.tax = "apweb", missing_action = "NA") %>%

 select(family, order, group)  %>%
  distinct() 

 final_test <- full_join(final_pls_be_final, hierarchy) %>%
...
length(unique(final_test$new_name)) 
 


#########@@@@@@@@@@@################## Einde poging 2 

test_try_tax2 <- try_taxonomic %>%
  filter(TRY_AccSpeciesName %in% c(not_included2$AccSpeciesName)) %>%
  select(TRY_AccSpeciesName, MatchedName) %>%
  distinct() %>%
  filter(str_count(MatchedName, "\\S+") > 1)

length(unique(test_try_tax2$TRY_AccSpeciesName))

temp6 <- try_taxonomic %>%
  filter(TRY_AccSpeciesName == "Adesmia volkmannii")

count <- test_try_tax2 %>%
  count(TRY_AccSpeciesName) %>%
  filter(n > 1)

temp <- test_try_tax2 %>%
  filter(TRY_AccSpeciesName %in% c(count$TRY_AccSpeciesName))

test$scientificName <- classification %>%
  filter(taxonID %in% c("wfo-0000511233")) 

length(unique(try_tax_clean2$scientificName)) #61551

...
cats_both2 <- cats_both %>% filter(Model_type == "raw_trait") %>%
  filter(study %notin% c("Aiello1992", "Aiello2011" , "Asefa", "Osuri")) %>%
  filter(dif_fit > -0.4) %>% filter(dif_fit < 0.5)
ggplot(cats_both2, aes(x = dif_fit, y = mean_dist)) +
  geom_point(alpha = 0.3, aes(colour = study)) +
  theme_classic() +
...
model <- lmer((dif_fit) ~ (dif_FDiv) + species_richness + mean_dist + (1|study), data = cats_both2)

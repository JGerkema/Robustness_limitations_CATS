...
species_list <- colnames(riva_veg)
...
riva_clean <- lst(riva_veg, riva_trait, species_list, trait_information)
...
processed_specieslvl <- prep_plotlvl(riva_veg, riva_trait, trait_information, 0.8)
...
save(processed_specieslvl, diversity_info, trait_information,
  file = "data/processed/Riva_processed.RData")
...
test <- cats_plotlvl(processed_specieslvl, trait_information, processed_specieslvl$plot_code_vector)

calculations_cwm_cwv <- processed_specieslvl$cwm_cwv_all_traits

figure <- cats_visualisation(test)
